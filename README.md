...
### Linter installation
Before using this plugin, you must ensure that `serplint` is installed on your system. To install `serplint`, do the following:

1. Install [Python](http://python.org/download/) and [pip](http://www.pip-installer.org/en/latest/installing.html).

1. Install `serplint` by typing the following in a terminal:
   ```
   [sudo] pip install serplint
   ```
...
Once you have installed and configured `serplint`, you can proceed to install the SublimeLinter-contrib-serplint plugin if it is not yet installed.
...
1. When the plugin list appears, type `serplint`. Among the entries you should see `SublimeLinter-contrib-serplint`. If that entry is not highlighted, use the keyboard or mouse to select it.
